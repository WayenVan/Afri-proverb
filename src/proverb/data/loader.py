...
from .processor import Processor, RawProcessor
from ..extras.misc import print_dataset_sample, print_dataset_sample_raw
...
def load_proverb_dataset(
    ...
    for loc, langs in data_args.location_langauge_paris:
        for lang in langs:
            ...
            processor = Processor(tokenizer, lang, loc, data_args, task_args, dataset)
...
def load_raw_dataset(
    ...
    for loc, langs in data_args.location_langauge_paris:
        for lang in langs:
            ...
            if training_args.local_process_index == 0:
                print(f"Sample from location: {loc}, language: {lang}\n")
                print_dataset_sample_raw(dataset, num_samples=2)
